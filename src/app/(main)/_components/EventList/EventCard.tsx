...
import MarketForEventCard from "./MarketForEvent";
...
const EventCard: React.FC<EventCardProps> = ({ event, changeWatchList }) => {
  ...
  useEffect(() => {
    const _markets: MarketInterface[] = sortByMarkets(event);
    const watchList = getCookie('watchlist');
    if (watchList) {
      if (watchList.split(',').includes(event.id)) {
        setIsWatchList(true);
      }
    }
    setMarkets(_markets);
  ...
  const setWatchList = () => {
    ...
    if (watchList) {
      if (watchList.split(',').includes(event.id)) {
        const _watchList = watchList.split(',').filter((item: string) => item !== event.id);
        setCookie('watchlist', _watchList, null);
      } else {
        watchList += ',' + event.id;
        setCookie('watchlist', watchList, null);
      }
    } else {
      setCookie('watchlist', event.id, null)
    }
  ...
  return (
    <>
      ...
      <div className="w-full h-full flex flex-col justify-between bg-black bg-[linear-gradient(107.7deg,rgba(0,0,0,0)_37.9%,rgba(96,4,213,0.2)_113.22%),linear-gradient(248.12deg,rgba(24,20,32,0)_35.67%,#292929_101.76%)] rounded-lg pt-5 shadow-md hover:shadow-lg">
        ...
        <div
          className={`flex flex-col gap-1 ${openMarket == true && selectedMarket != null
            ? "flex-grow"
          ...
          {openMarket == true && selectedMarket != null ? (
            ...
            markets?.length == 1 && (
              <div className="flex items-end gap-2 h-[75px] pb-2">
                ...
                <button
                  className="flex justify-center items-center gap-1 bg-[rgba(255,0,0,0.1)] hover:bg-[#BB0000] rounded-lg px-2 py-2 flex-1 text-red hover:text-white"
                  onClick={() =>
                    openMarketHandle(
                      markets[0],
        ...
        <div className="flex justify-between px-5 py-2 bg-[linear-gradient(0deg,rgba(35,34,36,0.8),rgba(35,34,36,0.8)),linear-gradient(248.12deg,rgba(24,20,32,0)_35.67%,#292929_101.76%),linear-gradient(106.24deg,rgba(0,0,0,0)_38.72%,rgba(96,4,213,0.5)_104.16%)] rounded-b-lg">
          ...
          <div className="flex gap-2 items-center">
            ...
            </div>
            {
              isWatchList ?
                <StarIcon
                  className="text-[17px] text-[#BFF816] cursor-pointer"
                  onClick={setWatchList}
                />
                :
                <StarOutlineIcon
                  className="text-[17px] text-neutral-500 cursor-pointer"
                  onClick={setWatchList}
                />
            }
          </div>
